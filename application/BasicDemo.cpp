...
BasicDemo::BasicDemo() :
		BulletOpenGLApplication(), m_bApplyForce(false), m_pExplosion(0), m_bCanExplode(
				true)
{
...
void BasicDemo::InitializePhysics()
{
	...
	// create the world
	m_pWorld = new btDiscreteDynamicsWorld(m_pDispatcher, m_pBroadphase,
...
void BasicDemo::CreateObjects()
{
	...
	m_pBox = CreateGameObject(new btBoxShape(btVector3(1, 1, 1)), 1.0,
...
void BasicDemo::CollisionEvent(btRigidBody* pBody0, btRigidBody* pBody1)
{
	...
	if (pBody0 == m_pExplosion || pBody1 == m_pExplosion)
	{
		...
		btRigidBody* pOther;
		pBody0 == m_pExplosion ? pOther = (btRigidBody*) pBody1 : pOther =
											(btRigidBody*) pBody0;
		// wake the object up
		...
		btVector3 dir = pOther->getWorldTransform().getOrigin()
				- m_pExplosion->getWorldTransform().getOrigin();
		// get the distance
...
void BasicDemo::Keyboard(unsigned char key, int x, int y)
{
...

	{
		...
		m_pExplosion->setCollisionFlags(
